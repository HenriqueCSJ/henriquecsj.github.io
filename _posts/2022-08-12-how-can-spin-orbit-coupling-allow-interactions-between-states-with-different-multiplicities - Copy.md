...
Crucially, there is no guarantee that the spin components $ \| \chi_{\text{spin}}^{(n)} \> $ all have the same value of $ S $. You may mix together singlets, triplets, ... all into the same state.
